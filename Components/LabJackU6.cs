...
namespace AeonHacs.Components
{
        ...
        LabJackU6.IConfig, LabJackU6.IDevice
    {
        ...
        static double RandomPlusOrMinus1 => 2.0 * Random.NextDouble() - 1.0;
        ...
        protected override void ManageState()
        {
            ...
            //    Device.ScansReceived += 1;
            //    ++Device.UpdatesReceived;

            //    for (int i = 0; i < ai.Count; i++)
            //        ai[i].Device.Voltage = 3 + 0.01 * RandomPlusOrMinus1;
            //    DataAcquired = true;
        ...
        {
            double ljResult = 0;
            try { LJUD.eGet(lj.ljhandle, LJUD.IO.GET_CONFIG, configChannel, ref ljResult, 0); }
            catch (Exception e) { Log?.Record($"configChannel {configChannel}: " + e.ToString()); }
            return ljResult;
        }
        ...
        void ConfigureStream()
        {
            ...
            try
            {
                ...
                //resolutionIndex = 0;        // TODO: Jim, why did we lock this to 0?
...
//                    ai[i].Device.Voltage = streamData[i + i];
                ai[i].Device.Voltage = streamData[i + i + 1];       // use the 2nd of the two readings
                DataAcquired = true;
