...
public class SerialController : StateManager, ISerialController
{
    ...
    protected override int StateLoopTimeout
    {
        get
        {
            ...
            {
                var mbm = SerialDevice.MillisecondsBetweenMessages;
                var minWait = Math.Max(5, mbm);
                if (mbm < 1) mbm = minWait;
                int n = (CommandMessages?.Length ?? 0) - 1;
                timeout = AwaitingResponses > 0 ? ResponseTimeout : minWait + n * mbm;
            }
            else
            {
                timeout = IdleTimeout;
