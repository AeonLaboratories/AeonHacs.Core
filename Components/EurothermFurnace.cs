...
namespace AeonHacs.Components
{
        ...
        EurothermFurnace.IConfig, EurothermFurnace.IDevice
    {
        ...
        [JsonProperty("SetpointRateLimit"), DefaultValue(0)]
        ...
        [JsonProperty("SetpointRateLimitUnits"), DefaultValue(SetpointRateLimitUnitsCode.Minutes)]
        ...
        [JsonProperty("OutputRateLimit"), DefaultValue(7)]
        ...
        ResolutionCode IDevice.Resolution { get => resolution; set => Ensure(ref resolution, value); }
        ResolutionCode resolution = ResolutionCode.Unknown;

        bool IDevice.SetpointRateLimitActive { get => setpointRateLimitActive; set => Ensure(ref setpointRateLimitActive, value); }
        bool setpointRateLimitActive;

        int IDevice.PVMinimum { get => pvMinimum; set => Ensure(ref pvMinimum, value); }
        int pvMinimum = -1;

        int IDevice.ParameterValue { get => parameterValue; set => Ensure(ref parameterValue, value); }
        int parameterValue = -1;

        InstrumentModeCode IDevice.InstrumentMode { get => instrumentMode; set => Ensure(ref instrumentMode, value); }
        InstrumentModeCode instrumentMode= InstrumentModeCode.Unknown;

        ControlTypeCode IDevice.ControlType { get => controlType; set => Ensure(ref controlType, value); }
        ControlTypeCode controlType = ControlTypeCode.Unknown;
        ...
        public new void TurnOn(double setpoint)
        {
            ...
            {
                Alert.DefaultAlertManager.Pause($"{Name} Error", "ControlType must be PID for setpoint control.");
                return;
            }
        ...
        [JsonProperty("DeviceAddress"), DefaultValue(2)]
        ...
        int check = 1, nchecks = 2; // skip setpoint rate limit checks
        string CheckStatus()
        {
            string command = "";
            switch (check)
        ...
        protected override bool ValidateResponse(string response, int which)
        {
            try
            {
                ...
                var cmd = command.ToByteString();
                var rsp = response.ToByteString();
                ...
                if (reportFunctionCode != commandFunctionCode)
                {
                    if (reportFunctionCode == commandFunctionCode + 128)
                    {
                        ...
                        var badParam = errorCode == ErrorResponseCode.IllegalDataAddress;
                        var badData = errorCode == ErrorResponseCode.IllegalDataValue;

                        // TODO: dynamically create log; do not require LogEverything here
                        if (LogEverything)
                            Log.Record("Error Reponse: " +
                                (
                                    badParam ? "Illegal parameter: [" :
                                    badData ? "Illegal parameter value: [" :
                                    "Unknown Error: ["
                                )
                                + cmd + "]");
                        return true;
                    ...
                    {
                        if (LogEverything)
                            Log.Record("Command/Report mismatch: [" + cmd + "]/[" + rsp + "]");
                    }
                    return false;
                }
                ...
                if (reportFunctionCode == FunctionCode.Read)
                {
                    ...
                    else if (firstParam == (int)ParameterCode.ControlType)
                    {
                        ...
                        if (LogEverything) Log.Record($"{(ParameterCode)firstParam} = {(ControlTypeCode)firstValue}");
                    }
                    else if (firstParam == (int)ParameterCode.InstrumentMode)
                    {
                        Device.InstrumentMode = (InstrumentModeCode)firstValue;
                        if (LogEverything) Log.Record($"{(ParameterCode)firstParam} = {(InstrumentModeCode)firstValue}");
                    }
                    ...
                    else if (firstParam == (int)ParameterCode.Resolution)
                    {
                        ...
                        if (LogEverything) Log.Record($"{(ParameterCode)firstParam} = {(ResolutionCode)firstValue}");
                    }
                    ...
                    else if (firstParam == (int)ParameterCode.SetpointRateLimitActiveStatus)
                    {
                        ...
                        if (LogEverything) Log.Record($"{(ParameterCode)firstParam} = {Device.SetpointRateLimitActive}");
                    }
                    else if (firstParam == (int)ParameterCode.SetpointRateLimitUnits)
                    {
                        ...
                        if (LogEverything) Log.Record($"{(ParameterCode)firstParam} = {(SetpointRateLimitUnitsCode)firstValue}");
                    }
